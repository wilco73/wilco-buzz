...
function BlurReveal({ children, blurred, style }) {
  ...
  return (
    <span
      ...
      style={{
        ...style,
        filter: blurred && !revealed ? 'blur(8px)' : 'none',
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{
            color: T.textDim, fontSize: 10, letterSpacing: 2, marginBottom: 4,
            textTransform: 'uppercase',
          }}>Code {streamerMode && '(clic pour révéler)'}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <BlurReveal blurred={streamerMode} style={{
              fontFamily: "'Orbitron', monospace", fontSize: 28,
              color: T.accent, letterSpacing: 8, fontWeight: 900,
            }}>{room.code}</BlurReveal>
        ...
        {room.password && (
          <div>
            <div style={{
              color: T.textDim, fontSize: 10, letterSpacing: 2, marginBottom: 4,
              textTransform: 'uppercase',
            }}>Mot de passe {streamerMode && '(clic)'}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <BlurReveal blurred={streamerMode} style={{
                fontFamily: "'Orbitron', monospace", fontSize: 20,
                color: T.yellow, letterSpacing: 4, fontWeight: 700,
              }}>{room.password}</BlurReveal>
              <CopyButton text={room.password} />
            </div>
          </div>
        )}
        ...
        {room.password && (
          <div>
            ...
            <CopyButton
              text={`Rejoins le buzz !\nCode : ${room.code}\nMot de passe : ${room.password}`}
              label="📤 Copier code + mdp"
            />
          </div>
        )}
      ...
      <div style={{
        ...
        <div style={{ marginBottom: timerIsActive ? 12 : 0 }}>
          <div style={{ color: T.textMuted, fontSize: 10, letterSpacing: 1, marginBottom: 8, textTransform: 'uppercase' }}>
            Durée
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <input
                ...
                style={{
                  width: 50, background: T.bg,
                  border: `1px solid ${T.border}`, borderRadius: 8,
                  color: T.text, padding: '6px 8px', fontSize: 11,
                  fontFamily: "'JetBrains Mono', monospace",
                  outline: 'none', textAlign: 'center',
                }}
              />
              <button onClick={handleCustomDuration} style={{
          ...
          <div style={{ color: T.textMuted, fontSize: 9, marginTop: 6 }}>
            ∞ = chrono libre. Sinon = décompte, buzzers désactivés à 0.
          </div>
        </div>
      ...
      <div style={{
        ...
        <button onClick={() => action('admin:new-round', { withTimer: true })} style={{
        ...
        <button onClick={() => action('admin:new-round', { withTimer: false })} style={{
        ...
        <button onClick={() => action('admin:clear-buzzes')} style={{
        ...
        <button onClick={() => action('admin:reset')} style={{
          background: T.surface, border: `2px solid ${T.border}`,
          borderRadius: 14, padding: 16, cursor: 'pointer', textAlign: 'center',
        }}>
          <div style={{ fontSize: 22, marginBottom: 4 }}>🗑️</div>
        ...
        {players.length === 0 ? (
          ...
          <div key={pid} style={{
            ...
            <span style={{
              ...
              background: p.manuallyBlocked
                ? 'rgba(156,39,176,0.15)'
                : p.buzzerDisabled
                  ? 'rgba(255,23,68,0.15)'
                  : 'rgba(0,230,118,0.15)',
              color: p.manuallyBlocked
                ? '#ce93d8'
                : p.buzzerDisabled
                  ? T.danger
                  : T.green,
              fontWeight: 700, letterSpacing: 1,
            }}>{p.manuallyBlocked ? '🔒 BLOQUÉ' : p.buzzerDisabled ? 'INACTIF' : 'ACTIF'}</span>
            {smallBtn(
              p.manuallyBlocked ? 'Débloquer' : 'Bloquer',
              p.manuallyBlocked ? T.green : T.orange,
              p.manuallyBlocked ? 'rgba(0,230,118,0.3)' : 'rgba(255,145,0,0.3)',
      ...
      <div style={{
        ...
        {showOverlay && (
          <div style={{ marginTop: 16 }}>
            ...
            <p style={{ color: T.textMuted, fontSize: 11, marginTop: 8 }}>500×700px recommandé</p>
          </div>
        )}
      </div>
        ...
        {showEmbed && (
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
              ...
              <CopyButton text={embedUrlWithPassword} />
            </div>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <div style={{
                flex: 1, minWidth: 200,
              ...
              <CopyButton text={`<iframe src="${embedUrlWithPassword}" style="width:100%;height:400px;border:none;"></iframe>`} />
            </div>
            <p style={{ color: T.textMuted, fontSize: 11, marginTop: 8, lineHeight: 1.6 }}>
              Ajouter <span style={{ color: T.text }}>?pseudo=X&avatar=🦊</span> pour auto-join.
            </p>
          </div>
        )}
      </div>
